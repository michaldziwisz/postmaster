...
class ChatControllerNode: ASDisplayNode, ASScrollViewDelegate {
    ...
    private func updateVoiceOverOverlayAccessibilityIsolation(viewControllers: [UIViewController]) {
        ...
        if resolvedIsChatOnTop, !overlay.shouldPreserveModalIsolationDuringFocusRecovery, let window = ChatControllerNode.resolveVoiceOverOverlayWindow(controller: controller, overlay: overlay), let windowHost = window as? WindowHost {
            ...
            windowHost.forEachController { candidate in
                if (candidate as AnyObject) === (controller as AnyObject) {
                    ...
                    let candidateBundle = Bundle(for: type(of: candidate))
                    let isApplicationController = candidateBundle == Bundle.main
                    if !isApplicationController {
                        return
                    }
        ...
        controller.viewIfLoaded?.accessibilityViewIsModal = resolvedIsChatOnTop
    ...
    func prepareForVoiceOverExternalPresentation() {
        ...
        self.controller?.viewIfLoaded?.accessibilityViewIsModal = false
    }
    ...
    func restoreVoiceOverOverlayAfterExternalPresentation(focusProfileButton: Bool) {
        ...
        overlay.accessibilityElementsHidden = false
        overlay.accessibilityViewIsModal = true
        self.controller?.viewIfLoaded?.accessibilityViewIsModal = true
    ...
    init(context: AccountContext, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, subject: ChatControllerSubject?, controllerInteraction: ChatControllerInteraction, chatPresentationInterfaceState: ChatPresentationInterfaceState, automaticMediaDownloadSettings: MediaAutoDownloadSettings, navigationBar: NavigationBar?, statusBar: StatusBar?, backgroundNode: WallpaperBackgroundNode, controller: ChatControllerImpl?) {
        ...
        self.navigateButtons = ChatHistoryNavigationButtons(strings: self.chatPresentationInterfaceState.strings, theme: self.chatPresentationInterfaceState.theme, preferClearGlass: self.chatPresentationInterfaceState.preferredGlassType == .clear, dateTimeFormat: self.chatPresentationInterfaceState.dateTimeFormat, backgroundNode: self.backgroundNode, isChatRotated: historyNodeRotated)
